...
public class ODataStorage {
    ...
    public ODataStorage(final OData odata, final Edm edm) {
        this.odata = odata;
        this.edm = edm;
        this.manager = new TransactionalEntityManager(edm);

        // final List<Entity> productList = manager.getEntityCollection(com.alvachien.libraryapi.util.Constants.ES_PERSONS_NAME);
    ...
    public Entity readFunctionImportEntity(final UriResourceFunction uriResourceFunction,
        ...
        return ODataUtil.findEntity(edmEntityType, entityCollection, uriResourceFunction.getKeyPredicates());
    }
    ...
    public EntityCollection readFunctionImportCollection(final UriResourceFunction uriResourceFunction,
        ...
        if (com.alvachien.libraryapi.edm.ODataConstants.FUNCTION_COUNT_CATEGORIES.equals(uriResourceFunction.getFunctionImport().getName())) {
            ...
            // for (final Entity category : manager.getEntityCollection(com.alvachien.libraryapi.util.Constants.ES_CATEGORIES_NAME)) {
            //     final EntityCollection products = getRelatedEntityCollection(category, com.alvachien.libraryapi.util.Constants.NAV_TO_PRODUCTS);
    ...
    public void resetDataSet(final int amount) {
        ...
        // final List<Entity> productList = manager.getEntityCollection(com.alvachien.libraryapi.util.Constants.ES_PERSONS_NAME);
        // final List<Entity> categoryList = manager.getEntityCollection(com.alvachien.libraryapi.util.Constants.ES_BOOKS_NAME);
    ...
    public Entity readEntityData(EdmEntitySet edmEntitySet, List<UriParameter> keyParams)
        ...
        if (edmEntitySet.getName().equals(com.alvachien.libraryapi.edm.ODataConstants.ES_PERSONS_NAME)) {
            return getEntity(edmEntityType, keyParams, manager.getEntityCollection(com.alvachien.libraryapi.edm.ODataConstants.ES_PERSONS_NAME));
        } else if (edmEntitySet.getName().equals(com.alvachien.libraryapi.edm.ODataConstants.ES_BOOKS_NAME)) {
            return getEntity(edmEntityType, keyParams, manager.getEntityCollection(com.alvachien.libraryapi.edm.ODataConstants.ES_BOOKS_NAME));
        }
    ...
    public Entity getRelatedEntity(Entity entity, UriResourceNavigation navigationResource)
        ...
        if (edmNavigationProperty.isCollection()) {
            return ODataUtil.findEntity(edmNavigationProperty.getType(),
    ...
    public Entity createEntityData(EdmEntitySet edmEntitySet, Entity entityToCreate, String rawServiceUri)
        ...
        if (edmEntitySet.getName().equals(com.alvachien.libraryapi.edm.ODataConstants.ES_PERSONS_NAME)) {
            return createEntity(edmEntitySet, edmEntityType, entityToCreate,
                    manager.getEntityCollection(com.alvachien.libraryapi.edm.ODataConstants.ES_PERSONS_NAME), rawServiceUri);
        } else if (edmEntitySet.getName().equals(com.alvachien.libraryapi.edm.ODataConstants.ES_BOOKS_NAME)) {
            return createEntity(edmEntitySet, edmEntityType, entityToCreate,
                    manager.getEntityCollection(com.alvachien.libraryapi.edm.ODataConstants.ES_BOOKS_NAME), rawServiceUri);
        }
    ...
    public void updateEntityData(EdmEntitySet edmEntitySet, List<UriParameter> keyParams, Entity updateEntity,
        ...
        if (edmEntitySet.getName().equals(com.alvachien.libraryapi.edm.ODataConstants.ES_PERSONS_NAME)) {
            updateEntity(edmEntityType, keyParams, updateEntity, httpMethod,
                    manager.getEntityCollection(com.alvachien.libraryapi.edm.ODataConstants.ES_PERSONS_NAME));
        } else if (edmEntitySet.getName().equals(com.alvachien.libraryapi.edm.ODataConstants.ES_BOOKS_NAME)) {
            updateEntity(edmEntityType, keyParams, updateEntity, httpMethod,
                    manager.getEntityCollection(com.alvachien.libraryapi.edm.ODataConstants.ES_BOOKS_NAME));
        }
    }
    ...
    public void deleteEntityData(EdmEntitySet edmEntitySet, List<UriParameter> keyParams)
        ...
        if (edmEntitySet.getName().equals(com.alvachien.libraryapi.edm.ODataConstants.ES_PERSONS_NAME)) {
            deleteEntity(edmEntityType, keyParams, manager.getEntityCollection(com.alvachien.libraryapi.edm.ODataConstants.ES_PERSONS_NAME));
        } else if (edmEntitySet.getName().equals(com.alvachien.libraryapi.edm.ODataConstants.ES_BOOKS_NAME)) {
            deleteEntity(edmEntityType, keyParams, manager.getEntityCollection(com.alvachien.libraryapi.edm.ODataConstants.ES_BOOKS_NAME));
        }
    }
    ...
    private String getEntitySetName(Entity entity) {
        if (com.alvachien.libraryapi.edm.ODataConstants.ET_PERSON_FQN.getFullQualifiedNameAsString().equals(entity.getType())) {
            return com.alvachien.libraryapi.edm.ODataConstants.ES_PERSONS_NAME;
        } else if (com.alvachien.libraryapi.edm.ODataConstants.ET_BOOK_FQN.getFullQualifiedNameAsString().equals(entity.getType())) {
            return com.alvachien.libraryapi.edm.ODataConstants.ES_BOOKS_NAME;
        }
